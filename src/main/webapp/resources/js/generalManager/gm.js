                    ...
                    columnDefs: [
                        ...
                        },
                        {
                            ...
                            "render": function (data, type, row) {
                                let temp;
                                if(row.statusCode === 3)
                                    temp = '<button class="changeProfession" value="3" id="' + row.professionNo + '">등록 완료</button>';
                                else
                                    temp = '<button class="changeProfession" value="4" id="' + row.professionNo + '">등록 취소</button>';
...
$(document).on("click", ".changeProfession", function (){
    const statusCode = Number.parseInt($(this).val());
    const thisProfessionNo = $(this).attr("id");

    if(statusCode === 3){
        if(confirm("이 전문가를 등록 완료 상태로 변경하시겠습니까?"))
            changePro(thisProfessionNo, statusCode);
        else
            alert("취소하셨습니다.")
